...
const compute = (originalInput, replacement1, replacement2) => {
  ...
  while (input[position] !== 99) {
    const x = input[position + 1];
    const y = input[position + 2];
    const outputIndex = input[position + 3];
    input[outputIndex] = input[position] === 1 ? input[x] + input[y] : input[x] * input[y];
    position += 4;
  }
  return input[0]
};

compute(originalInput, 12, 2);


const findWords = (input, result) => {

   for (let verb = 0; verb <= 99; verb++) {
      for (let noun = 0; noun <= 99; noun++) {
        let extract = compute(input, noun, verb);
        if (extract === result) {
...
findWords(originalInput, result);
