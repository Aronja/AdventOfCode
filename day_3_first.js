...
const wire_one = input_wire_one.split(",");
const wire_two = input_wire_two.split(",");

const startingPoint = {"R": 0, "L":0, "U":0, "D":0};

let location_wire_one = startingPoint;
let location_wire_two = startingPoint;

const calculateNewLocation = (input, currentLocation) => {
  const directions = ["R", "L", "U", "D"];
  const inputDirection = input.slice(0,1);
  const directionToTurn = directions[directions.map(x => x.includes(inputDirection)).indexOf(true)];
  console.log(findOpposite(input));
  const oppositeDirectionToTurn = directions[directions.map(x => x.includes(findOpposite(input))).indexOf(true)];
  currentLocation[directionToTurn] += parseInt(input.slice(1));
  currentLocation[oppositeDirectionToTurn] -= parseInt(input.slice(1));
  return currentLocation;
};

const findOpposite = (input) => {
  const directionMap = {"R" : "L", "L" : "R", "U" : "D", "D" : "U"};
  return directionMap[input.slice(0,1)]
}

const repeatForWire = (wire_one) => {
  let currentLocation;
  for (let i =0; i < wire_one.length; i++) {
    const newLocation = calculateNewLocation(wire_one[i], startingPoint);
